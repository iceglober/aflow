...
**Source:** /think + /spec-refine x2 + /spec-enrich + /spec-review sessions, 2026-03-30

## Changelog

### v5 — simplicity refinement (2026-03-30)

- **OQ-06 decided:** `/work-backlog` is killed. Its behavior merges into `/work`, which reads from `af state` when a task exists.
- **Adoption simplicity pass:**
  - User-facing surface reduced to 3 commands: `af skills`, `af start`, `af status`. Everything else is plumbing.
  - `af state` reclassified as internal/advanced — exists as a command (Claude needs it) but not prominently documented.
  - `.aflow/state/` and `.aflow/specs/` auto-created by `af state` on first write. No init step.
  - `.aflow/state/` auto-added to `.gitignore` on first run. Specs stay committed.
  - Config system deferred entirely. No `config.json`, no `af config`. `--quick` is just a flag.
- **Requirements changed:**
  - R-24 updated: `/work-backlog` removed from affected skills, `/work` added
  - Added R-25 (auto-gitignore)
  - Added R-26 (auto-create directories)
  - Added R-27 (user-facing command surface)
- **Migration map updated:** `work-backlog.ts` moved to Delete section

### v4 — spec review (2026-03-30)
- Added `cancelled` phase, BR-09/10/11, QAResult schema, workstream ID format, canonical skill names
- Fixed R-17 skip logic, R-11 resume behavior
...
- Configuration system (`af config`, `config.json`) — deferred, flags are sufficient for v1
...
- **Spec:** A structured Markdown document produced during the design phase. Lives at `.aflow/specs/{task-id}.md` in the main repo. Accessed exclusively via `af state spec` commands. Committed to git (shared artifact).
- **State:** The structured, machine-readable record of a task's current phase, metadata, and history. Owned exclusively by `af state` — no other process writes to it. Stored as individual JSON files in `.aflow/state/`. Gitignored (per-engineer, not shared).
...
**R-27:** The user-facing commands MUST be limited to three: [MUST]

| Command | Purpose | When to use |
|---|---|---|
| `af skills [--force]` | Install aflow skills into the repo | Once per repo (or to update) |
| `af start ["description"]` | Start or resume a task pipeline | To begin work or pick up where you left off |
| `af status` | See all tasks, phases, and progress | To check what's in flight |

`af state` exists as a command for Claude and advanced users but is NOT part of the primary adoption story. Help text lists it under "Advanced" or "Internals," not alongside the main commands. `af wt` subcommands remain available for manual worktree management.
...
**R-25:** On first run, `af start` (via `af state`) MUST auto-add `.aflow/state/` to the repo's `.gitignore` if not already present. `.aflow/specs/` MUST NOT be gitignored — specs are shared artifacts. [MUST]

**R-26:** `af state` MUST auto-create `.aflow/state/` and `.aflow/specs/` directories on first write. No manual init step, no `af init` command. [MUST]
...
**R-24:** The `TASK_PREAMBLE` (`src/skills/preamble.ts`) MUST be rewritten to use `af state` instead of reading `backlog.json`. The 4 affected skills are: `/think`, `/work`, `/fix`, `/qa`, `/ship`. The new preamble MUST instruct Claude to: [MUST]
...
The unaffected skills (`spec-make`, `spec-refine`, `spec-enrich`, `spec-review`, `spec-lab`, `research-web`, `research-auto`, `browser`) need no changes.
...
```
.aflow/
├── state/                   # GITIGNORED — per-engineer workflow state
...
│   ├── t2.json
│   └── ...
├── specs/                   # COMMITTED — shared design artifacts
...
$ cd my-repo
$ af skills               # installs Claude skills into .claude/commands/
$ af start                # prompts: "What are you working on?"
                          # → launches the full pipeline
                          # → you talk to Claude, approve the design, it builds and ships
$ af status               # see what's in flight
```

That's it. No config files. No init commands. No directory setup. No state management. Just `af skills` and `af start`.
...
- `src/skills/work-backlog.ts` — killed, behavior merged into `/work`
...
- `src/skills/work.ts` — absorb `/work-backlog` behavior: if `af state` has an active task, read items from it; if not, work ad-hoc
- `src/index.ts` — add `state` and `status` subcommand groups, replace `start` command
- `src/help.ts` — add help text for `af start`, `af status`; `af state` under "Advanced" section
...
- `src/lib/state.ts` — state read/write/validate logic (includes auto-create dirs, auto-gitignore)
...
- `src/skills/` — all other skill source files unchanged (only preamble.ts and work.ts change)
